...
class ConversationMongoAdapterTest {
	...
	void save_success() {
		...
		ConversationTurn turn = ConversationTurn.create("안녕하세요");
		ConversationEntity savedEntity = new ConversationEntity("id-123", "안녕하세요", null, now);
	...
	void save_withResponse_success() {
		Instant now = Instant.now();
		ConversationTurn turn = ConversationTurn.withId("id-456", "질문", "답변", now);
		ConversationEntity savedEntity = new ConversationEntity("id-456", "질문", "답변", now);
	...
	void findRecent_success() {
		...
		ConversationEntity entity1 = new ConversationEntity("id-1", "첫 번째", "응답1", time1);
	...
	void findAll_success() {
		...
		when(mongoRepository.findAll()).thenReturn(Flux.just(entity1, entity2));

		StepVerifier.create(adapter.findAll()).assertNext(result -> {
			assertThat(result.id()).isEqualTo("id-1");
	...
	void save_error_propagates() {
		ConversationTurn turn = ConversationTurn.create("에러 테스트");
