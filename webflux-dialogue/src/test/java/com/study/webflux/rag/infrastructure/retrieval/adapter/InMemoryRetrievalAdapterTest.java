...
class InMemoryRetrievalAdapterTest {
	...
	void retrieve_keywordMatch_success() {
		...
		when(conversationRepository.findAll()).thenReturn(Flux.just(turn1, turn2, turn3));

		StepVerifier.create(adapter.retrieve("스프링 웹플럭스", 5)).assertNext(context -> {
	...
	void retrieve_sortedByScore() {
		...
		StepVerifier.create(adapter.retrieve("자바 스프링", 5)).assertNext(context -> {
	...
	void retrieve_topK_limit() {
		...
		when(conversationRepository.findAll()).thenReturn(Flux.just(turn1, turn2, turn3, turn4));

		StepVerifier.create(adapter.retrieve("자바", 2)).assertNext(context -> {
			assertThat(context.documents()).hasSize(2);
		}).verifyComplete();
	}
	...
	void retrieve_irrelevant_filtered() {
		...
		StepVerifier.create(adapter.retrieve("자바 스프링", 10)).assertNext(context -> {
	...
	void retrieve_empty_conversations() {
		when(conversationRepository.findAll()).thenReturn(Flux.empty());

		StepVerifier.create(adapter.retrieve("테스트", 5)).assertNext(context -> {
	...
	void retrieve_noRelevant_emptyResult() {
		...
		when(conversationRepository.findAll()).thenReturn(Flux.just(turn1, turn2));
	...
	void retrieve_caseInsensitive() {
		...
		when(conversationRepository.findAll()).thenReturn(Flux.just(turn));

		StepVerifier.create(adapter.retrieve("java spring", 5)).assertNext(context -> {
			assertThat(context.documents()).hasSize(1);
			assertThat(context.documents().get(0).score().value()).isEqualTo(2);
		}).verifyComplete();
	}
	...
	void retrieve_tokenization() {
		...
		StepVerifier.create(adapter.retrieve("스프링 자바", 5)).assertNext(context -> {
			assertThat(context.documents()).hasSize(1);
			assertThat(context.documents().get(0).score().value()).isEqualTo(2);
		}).verifyComplete();
	}
	...
	void retrieveMemories_returnsEmpty() {
		StepVerifier.create(adapter.retrieveMemories("테스트", 5)).assertNext(result -> {
	...
	void retrieve_intersectionSize() {
		...
		StepVerifier.create(adapter.retrieve("a b", 5)).assertNext(context -> {
			assertThat(context.documents()).hasSize(3);
			assertThat(context.documents().get(0).score().value()).isEqualTo(2);
	...
	void retrieve_error_propagates() {
		when(conversationRepository.findAll()).thenReturn(
			Flux.error(new RuntimeException("DB error")));

		StepVerifier.create(adapter.retrieve("테스트", 5))
