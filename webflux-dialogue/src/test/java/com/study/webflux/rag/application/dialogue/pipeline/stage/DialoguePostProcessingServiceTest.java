...
class DialoguePostProcessingServiceTest {
	...
	void persistAndExtract_shouldSaveConversationWithResponse() {
		...
		ConversationTurn turn = ConversationTurn.create(userId, "질문");
		PipelineInputs inputs = new PipelineInputs(userId, null, null, null, turn);

		when(pipelineTracer.tracePersistence(any())).thenAnswer(invocation -> {
			return Mono.just(turn.withResponse("응답 문장1 응답 문장2"));
		});
		when(conversationCounterPort.increment(userId)).thenReturn(Mono.just(1L));

		Flux<String> sentences = Flux.just("응답 문장1", "응답 문장2");

		StepVerifier.create(service.persistAndExtract(Mono.just(inputs), sentences))
			.verifyComplete();

		verify(pipelineTracer).tracePersistence(any());
		verify(conversationCounterPort).increment(userId);
	}
	...
	void persistAndExtract_shouldIncrementConversationCounter() {
		...
		when(pipelineTracer.tracePersistence(any())).thenAnswer(invocation -> {
			return Mono.just(turn.withResponse("응답"));
		});
		when(conversationCounterPort.increment(userId)).thenReturn(Mono.just(3L));
	...
	void persistAndExtract_shouldTriggerMemoryExtractionAtThreshold() {
		...
		when(conversationCounterPort.increment(userId)).thenReturn(Mono.just(5L));
		when(memoryExtractionService.checkAndExtract(userId)).thenReturn(Mono.empty());

		StepVerifier.create(service.persistAndExtract(Mono.just(inputs), Flux.just("응답")))
			.verifyComplete();

		verify(memoryExtractionService).checkAndExtract(userId);
	}
