...
@Import(CreditControllerTest.TestPaymentConfig.class)
...
class CreditControllerTest {
	...
	/** PaymentGatewayPort Map 빈을 테스트용으로 제공 */
	@Configuration
	static class TestPaymentConfig {
		@Bean
		public Map<String, PaymentGatewayPort> paymentGatewayMap(PaymentGatewayPort tossGateway) {
			when(tossGateway.getProviderName()).thenReturn("toss");
			return Map.of("toss", tossGateway);
		}
	}
	...
	class GetBalance {
		...
		@Test
		@DisplayName("유효한 userId로 조회하면 200과 잔액을 반환한다")
		...
		@Test
		@DisplayName("크레딧 레코드가 없으면 잔액 0을 반환한다")
	...
	@Nested
	@DisplayName("GET /credit/transactions")
	class GetTransactions {

		@Test
		@DisplayName("트랜잭션 목록을 최신순으로 반환한다")
		...
		@Test
		@DisplayName("트랜잭션이 없으면 빈 배열을 반환한다")
		...
		@Test
		@DisplayName("페이지/사이즈 파라미터가 반영된다")
	...
	@Nested
	@DisplayName("POST /credit/charge/payment")
	class ChargeByPayment {

		@Test
		@DisplayName("유효한 PG 결제 요청 시 201과 충전된 트랜잭션을 반환한다")
		...
		@Test
		@DisplayName("지원하지 않는 PG사는 400을 반환한다")
		...
		@Test
		@DisplayName("amount가 0 이하인 요청은 400을 반환한다")
