...
public class DialoguePipelineService implements DialoguePipelineUseCase {
	...
	@Override
	@MonitoredPipeline
	public Flux<byte[]> executeAudioStreaming(ExecuteAudioDialogueCommand command) {
		AudioFormat targetFormat = command.audioFormat() != null
			? command.audioFormat()
			: defaultAudioFormat;

		Mono<PipelineInputs> inputsMono = inputService
			.prepareInputs(command.session(), command.text())
			.cache();
		...
		Flux<byte[]> audioFlux = ttsStreamService.buildAudioStream(
			new TtsSynthesisCommand(sentences, ttsWarmup, targetFormat,
				command.session().personaId()));
	...
	@Override
	@MonitoredPipeline
	public Flux<String> executeTextOnly(ExecuteTextDialogueCommand command) {
		Mono<PipelineInputs> inputsMono = inputService
			.prepareInputs(command.session(), command.text())
			.cache();
