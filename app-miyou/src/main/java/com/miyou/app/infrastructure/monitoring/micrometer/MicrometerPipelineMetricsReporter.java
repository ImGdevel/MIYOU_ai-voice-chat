...
public class MicrometerPipelineMetricsReporter implements PipelineMetricsReporter {
	...
	private void recordLlmMetrics(PipelineSummary summary) {
		for (StageSnapshot stage : summary.stages()) {
			...
			if (stage
				.status() == com.miyou.app.domain.monitoring.model.StageStatus.COMPLETED) {
				llmMetrics.recordLlmSuccess(modelTag);
			} else if (stage
				.status() == com.miyou.app.domain.monitoring.model.StageStatus.FAILED) {
	...
	private void recordUxMetrics(PipelineSummary summary) {
		...
		// 에러 기록
		if (summary
			.status() == com.miyou.app.domain.monitoring.model.PipelineStatus.FAILED) {
