...
class LoadBalancedSupertoneTtsAdapterTest {
	...
	fun setUp() {
		...
		adapter = LoadBalancedSupertoneTtsAdapter(
	...
	fun streamSynthesizeSuccess() {
		...
		StepVerifier.create(adapter.streamSynthesize("Hello, world!"))
	...
	fun streamSynthesizeTextTooLong() {
		...
		StepVerifier.create(adapter.streamSynthesize("a".repeat(301)))
			.expectError()
			.verify()
	}
	...
	fun concurrentRequestsLoadBalancedCorrectly() {
		...
		repeat(requestCount) { index ->
			adapter.streamSynthesize("Test $index")
				.doOnComplete {
	...
	private fun startFakeServer() {
		...
		server = HttpServer.create().port(TEST_PORT).handle(handlerAdapter).bindNow()
	}
	...
	private fun requestCount(vararg apiKeys: String): Int =
		apiKeys.sumOf { fakeServer.getRequestCount(it) }
