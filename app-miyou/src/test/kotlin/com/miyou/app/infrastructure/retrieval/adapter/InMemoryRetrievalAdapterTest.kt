...
class InMemoryRetrievalAdapterTest {
	...
	fun retrieve_keywordMatch_success() {
		...
		StepVerifier.create(adapter.retrieve(sessionId, "spring boot tips", 5))
			.assertNext { context ->
	...
	fun retrieve_empty_conversations() {
		...
		StepVerifier.create(adapter.retrieve(sessionId, "test", 5))
	...
	fun retrieveMemories_returnsEmpty() {
		...
		StepVerifier.create(adapter.retrieveMemories(sessionId, "test", 5))
