...
class CreditControllerTest {
	...
	fun getBalance_valid_returns200WithBalance() {
		...
		webTestClient.get().uri("/credit/balance?userId={id}", userId.value()).exchange()
			.expectStatus().isOk
			.expectBody()
	...
	fun getTransactions_returns200WithList() {
		...
		webTestClient.get().uri("/credit/transactions?userId={id}", userId.value()).exchange()
			.expectStatus().isOk
			.expectBody()
	...
	fun chargeByPayment_valid_returns201() {
		...
		val tx: CreditTransaction = CreditTransaction.of(
			userId,
		...
		webTestClient.post().uri("/credit/charge/payment")
