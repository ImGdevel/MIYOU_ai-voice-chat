...
class FakeSupertoneServer {
    ...
    fun getRequestCount(apiKey: String): Int = requestCounts.getOrDefault(apiKey, AtomicInteger(0)).get()
    ...
    private fun handleTtsRequest(request: ServerRequest): Mono<ServerResponse> {
        val apiKey =
            request.headers().firstHeader("x-sup-api-key")
        ...
        return request.bodyToMono(Map::class.java).flatMap { body ->
            ...
            when {
                ...
                text.length > 300 -> {
                    ...
                    behavior.apply(request, text)
                }
            }
        }
    }

    private fun handleHealthCheck(request: ServerRequest): Mono<ServerResponse> = ServerResponse.ok().build()
    ...
    fun interface ServerBehavior {
        ...
        companion object {
            ...
            @JvmStatic
            fun error(status: HttpStatus): ServerBehavior =
                ServerBehavior { _, _ ->
                    ServerResponse.status(status).bodyValue("Error: ${status.reasonPhrase}")
                }

            @JvmStatic
            fun delayed(delayMillis: Long): ServerBehavior =
                ServerBehavior { request, text ->
                    Mono.delay(Duration.ofMillis(delayMillis)).then(success().apply(request, text))
                }

            @JvmStatic
            fun timeout(): ServerBehavior =
                ServerBehavior { _, _ ->
                    ServerResponse
                        .ok()
                        .contentType(MediaType.parseMediaType("audio/wav"))
